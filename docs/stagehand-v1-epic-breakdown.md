...
- To do:
  ...
  - [x] raise recorded errors in `replay` mode
...
Y1 adds a Python `@stagehand.tool(...)` decorator for sync and async local functions. Record mode captures tool interactions under the `stagehand.tool` service with call arguments, result or error, timing, side-effect metadata, replay policy, parent/child links for nested calls, and scrubbed payloads. Replay mode returns recorded results, raises recorded errors, and fails closed on missing tool calls without invoking the wrapped implementation.
